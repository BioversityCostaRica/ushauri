...
__all__ = ["addLibrary", "addJSResource", "addCSSResource", "need"]
...
def resourceIndex(libraryIndex, resourceID, resourceType="JS"):
    if resourceType == "JS":
        for index in range(0, len(_LIBRARIES[libraryIndex]["JSResources"])):
            if (
                _LIBRARIES[libraryIndex]["JSResources"][index]["resourceID"]
                == resourceID
            ):
                return index
    if resourceType == "CSS":
        for index in range(0, len(_LIBRARIES[libraryIndex]["CSSResources"])):
            if (
                _LIBRARIES[libraryIndex]["CSSResources"][index]["resourceID"]
                == resourceID
            ):
                return index
    return -1
...
def addResourceToLibrary(
    libraryIndex, resourceType, resourceID, resourceFile, depends=None
):
    if depends == None:
        ...
        toResource = _LIBRARIES[libraryIndex]["library"].findall(
            ".//" + resourceType + "[@resourceID='" + depends + "']"
        )
        if toResource:
            resource = etree.Element(resourceType)
            resource.set("resourceID", resourceID)
    ...
    if resourceType == "JS":
        _LIBRARIES[libraryIndex]["JSResources"].append(
            {"libraryIndex": libraryIndex, "resourceID": resourceID}
        )
    else:
        _LIBRARIES[libraryIndex]["CSSResources"].append(
            {"libraryIndex": libraryIndex, "resourceID": resourceID}
        )


def addResource(
    libraryName, resourceID, resourceFile, resourceType="JS", depends="CHAIN"
):
                ...
                if resourceType == "JS":
                    if len(_LIBRARIES[index]["JSResources"]) == 0:
                        addResourceToLibrary(
                            index, resourceType, resourceID, resourceFile
                        )
                    else:
                        if depends == "CHAIN":
                            addResourceToLibrary(
                            ...
                            if depends is None:
                                ...
                                if depIndex >= 0:
                                    addResourceToLibrary(
                                        ...
                                        resourceID,
                                        resourceFile,
                                        _LIBRARIES[index]["JSResources"][depIndex][
                                            "resourceID"
                                        ],
                                    )
                                else:
                            ...
                            if depends is None:
                                ...
                                if depIndex >= 0:
                                    addResourceToLibrary(
                                        ...
                                        resourceID,
                                        resourceFile,
                                        _LIBRARIES[index]["CSSResources"][depIndex][
                                            "resourceID"
                                        ],
                                    )
                                else:
        ...
        if resource:
            ...
            for ancestor in resource[0].iterancestors():
                if ancestor.tag != "root":
                    ...
                    ancestors.insert(
                        0, {"resourceID": ancestorID, "filePath": filePath}
                    )
            ancestors.append(
                {
                    "resourceID": resourceID,
                    "filePath": os.path.join(
                        libraryPath, resource[0].get("resourceFile")
                    ),
                }
            )
    return ancestors
